import makeWASocket, { areJidsSameUser, DisconnectReason, isJidGroup, useMultiFileAuthState } from '@adiwajshing/baileys'
...
const connectToWhatsApp = async () => {
    ...
    client.ev.on('messages.upsert', async event => {
        for (const message of event.messages) {
            ...
            // If so, get the group
            const group = isGroup ? await client.groupMetadata(message.key.remoteJid) : undefined
            // Is the sender an admin of the group?
            const isAdmin = group ? group.participants.find(p => areJidsSameUser(p.id, sender))?.admin?.endsWith('admin') !== null : false
            // Is the Bot an admin of the group?
            const amAdmin = group ? group.participants.find(p => areJidsSameUser(p.id, client.user?.id))?.admin?.endsWith('admin') : false
            // Is sender banned?
